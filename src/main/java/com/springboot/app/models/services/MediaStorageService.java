...
public class MediaStorageService {
	  ...
	  @Value("${app.s3.presignMinutes}") private int presignMinutes;
	  @Value("${app.s3.maxSizeBytes}") private long maxSizeBytes;
	  ...
	  public String buildStorageKey(Long ownerId, String ext) {
		    ...
		    return "users/%d/%04d/%02d/%02d/%s.%s".formatted(
		        ownerId, today.getYear(), today.getMonthValue(), today.getDayOfMonth(),
	  ...
	  public void backendUpload(String storageKey, byte[] bytes, String contentType) {
		    s3.putObject(PutObjectRequest.builder()
		            .bucket(bucket).key(storageKey).contentType(contentType).build(),
